...
public class SeamCarverVisualizer {
    protected static final String IMAGE_NAME = "diagonals.png";
    protected static final int NUM_PIXELS = 150;
    private static final Orientation ORIENTATION = VERTICAL;
    JFrame frame;
    ...
    private void paintHorizontalSeam(Picture p, List<Integer> seam) {
        for (int i = 0; i < seam.size(); i++) {
            p.set(i, seam.get(i), new Color(203, 172, 199));
        }
    }

    private void paintVerticalSeam(Picture p, List<Integer> seam) {
        for (int i = 0; i < seam.size(); i++) {
            p.set(seam.get(i), i, new Color(0, 255, 144));
        }
    }
